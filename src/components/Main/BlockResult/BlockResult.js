...
const BlockResult = (props) => {
  ...
  if (SCORE === gameParams.maxGameScore) {
    ...
    SCORE >= gameParams.mediumGameScore &&
    SCORE < gameParams.maxGameScore
  ) {
  ...
  return (
    <div className="blockResult">
      <div className="greatingsBlock">
        ...
        <audio src={sound} autoPlay="autoplay" />
      </div>
      <Grid className="results blockShadow" container justify="center">
        <h3 className="h3">{congrats}</h3>
        <Grid container alignItems="center" justify="center" item xs={12}>
          <p className="h5-params result-text">
            Ты набрал {declOfNum(SCORE, BALL_STRING)} из{' '}
            {gameParams.maxGameScore} возможных
          </p>
        </Grid>
